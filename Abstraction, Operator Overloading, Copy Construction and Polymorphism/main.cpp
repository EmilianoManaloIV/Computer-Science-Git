...
Table::Table(const Table &a){
    szRow = a.szRow;
    szCol = a.szCol;
    columns = new RowAray*[a.szCol];
    for(int col=0; col < szCol; col++)
    {
        columns[col] = new RowAray(*a.columns[col]);
    }
}
...
PlusTab PlusTab::operator+(const PlusTab &a){
    PlusTab result(szRow,szCol);
    for (int col=0; col < szCol; col++)
    {
        for(int row=0; row < szRow; row++)
       {
           result.columns[col]->setData(row,(a.columns[col]->getData(row) + columns[col]->getData(row)));//columns[col]->setData(row, (a.columns[col]->getData(row) + columns[col]->getData(row)));
       }
    }
    return result;
}
