...
TriMatx *fillMat(int rows){
    ...
    triMt->size=rows;
    triMt->col=(int *)malloc(triMt->size * sizeof(int));
    triMt->indx=(int *)malloc(triMt->size * sizeof(int));
    
    triMt->data=(int **)malloc(triMt->size * sizeof(int *));
  for(int i=0;i < triMt->size; i++)
  {
      triMt->col[i] = mrkRand()%9+1;
      triMt->indx[i]=i;
      triMt->data[i] = (int *)malloc(triMt->col[i] * sizeof(int));
  }
  for(int i=0; i<triMt->size;i++)
  {
