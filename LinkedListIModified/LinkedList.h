...
T LinkedList<T>::pop_back()
{
    ...
    //Delete the popped data and create a new end for the list
    Link removedEnd = end;
    temp = front;
    int n = 0;
    do{
...
void LinkedList<T>::push_front(const T &Element)
{
    ...
    //Link the new element of the list
    newLink->data=Element;
    newLink->linkPtr=temp;
    front=newLink;
}
template<class T>
void LinkedList<T>::push_back(const T &Element)
{
    ...
    Link *newLink=new Link;        //Create the new link
    newLink->data=Element;            //Add the data
    newLink->linkPtr=NULL;         //Set the pointer to NULL
    end=newLink;         //Point to the new end of the list
}
template<class T>
